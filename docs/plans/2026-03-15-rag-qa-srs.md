...
While `rag.mode=agentic`，系统 shall 通过 LLM tool-calling 自主决定调用 KB检索/Web搜索/直答工具（单次或多轮）回答用户问题，而非固定 linear 流水线。If agent 在总超时（默认 30s）内未生成回答，then 系统 shall 降级到 linear RAG 保证主流程不中断。If 知识库无 COMPLETED 文档，then 系统 shall 允许 agent 调用 Web搜索/直答工具回答，而非直接返回"暂无文档"。
