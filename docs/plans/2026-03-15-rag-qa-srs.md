...
### 3.5 Agentic RAG（2026-07-03 增量 · Wave 1）

> 增量信号：`increment-request.json`。详见设计文档 §11。PoC 已验证（`MiniMaxToolCallingPoCTest` 4 用例全过）。

#### FR-012: Agentic 问答模式

While `rag.mode=agentic`，系统 shall 通过 LLM tool-calling 自主决定调用 KB检索/Web搜索/直答工具（单次或多轮）回答用户问题，而非固定 linear 流水线。If agent 在总超时（默认 30s）内未生成回答，then 系统 shall 降级到 linear RAG 保证主流程不中断。If 知识库无 COMPLETED 文档，then 系统 shall 允许 agent 调用 Web搜索/直答工具回答，而非直接返回"暂无文档"。系统 shall 支持 per-conversation 模式切换：用户可在对话级选择 linear/agentic（`Conversation.rag_mode`），覆盖全局 `rag.mode` 默认值；`rag_mode=null` 时继承全局默认。

**验收标准：**
- Given: rag.mode=agentic 且知识库有文档
- When: 用户提问"产品A和竞品X价格对比"
- Then: agent 自主调用 KB检索 + Web搜索（≥2 次 tool），返回基于两者的回答
- Given: rag.mode=agentic 且知识库无文档
- When: 用户提问
- Then: agent 调用 Web搜索/直答工具回答，不返回"暂无文档"
- Given: agent 总超时触发
- When: 超时（默认 30s）
- Then: 降级到 linear RAG 返回回答
- Given: rag.mode=linear
- When: 用户提问
- Then: 走现有 RagService 流水线，行为不变
- Given: conversation.rag_mode=agentic 且全局 rag.mode=linear
- When: 用户提问
- Then: 走 AgenticRagService（conversation 级覆盖全局）
- Given: conversation.rag_mode=null
- When: 用户提问
- Then: 用全局 rag.mode 默认值
- Given: 用户在对话切换 mode
- When: 调 PATCH /api/conversations/{id}/rag-mode
- Then: Conversation.rag_mode 持久化，后续提问按新 mode 路由
...
**验收标准：**
- Given: KnowledgeBaseSearchTool 已注册
- When: agent 调用 searchKnowledgeBase(query)
- Then: 复用 RagService.retrieve（召回+rerank+fallback），返回 RetrievalResult
- Given: TAVILY_API_KEY 已配置
- When: agent 调用 searchWeb(query)
- Then: 返回 Tavily top-N 摘要
- Given: TAVILY_API_KEY 未配置
- When: agent 启动
- Then: WebSearchTool 不可用，agent 仅用 KB/直答工具
- Given: 闲聊/常识类问题
- When: agent 判断无需检索
- Then: 调用 DirectAnswerTool 直接回答，不触发 KB/Web 检索

**优先级：** Must

---

#### FR-014: Agent 可观测与 trace 落库

系统 shall 将 agent 每轮 tool 调用（tool名/参数/结果摘要/耗时/轮次）记录到 agent_trace 表，关联 chat_history.id。When agent 执行一轮 tool 调用，系统 shall 通过 SSE 推送 agent_step 事件（含 tool名/状态/耗时），供前端展示思考过程。

**验收标准：**
- Given: agent 完成一次问答（N 次 tool 调用）
- When: 查询 agent_trace 表
- Then: 该 chat_id 下有 N 行 trace，每行含 tool_name/tool_args/result_summary/duration_ms/round
- Given: 流式问答（rag.mode=agentic）
- When: agent 调用 tool
- Then: SSE 推送 agent_step 事件（start/done 两态）
- Given: agent 降级到 linear
- When: 降级触发
- Then: chat_history.rag_metadata 记录 degraded=true、agent_mode=linear
